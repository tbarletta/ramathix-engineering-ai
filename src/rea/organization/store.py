...
from .contracts import OrganizationPlan, Rfc
...
class RfcStore:
    ...
    def save(self, rfc: Rfc) -> Path:
        self.root.mkdir(parents=True, exist_ok=True)
        path = self.root / f"{rfc.id}.json"
        path.write_text(
            json.dumps(rfc.to_dict(), ensure_ascii=False, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        return path

    def load(self, rfc_id: str) -> Rfc:
        path = self._path(rfc_id)
        return Rfc.from_dict(json.loads(path.read_text(encoding="utf-8")))
    ...
    def _path(self, rfc_id: str) -> Path:
        if not rfc_id or "/" in rfc_id or "\\" in rfc_id or ".." in rfc_id:
            raise ValueError("invalid RFC id")
        path = self.root / f"{rfc_id}.json"
        if not path.is_file():
            raise FileNotFoundError(rfc_id)
        return path
